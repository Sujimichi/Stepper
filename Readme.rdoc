...
You have loads of step definitions and your pretty sure that some are not being used anymore.  With a test suite that takes 30 minutes to run trail and error is not your friend, so how do you find your obselete steps?

    step --notused


Stepper was written in haste to solve a problem.  It could do with improvment!
...
Note the step must be wrapped in "" and any " in the step need to be escaped; <pre><code> \" </code></pre>
...
To get an output that shows for each step, where it is and which features use it (or if its not used)
